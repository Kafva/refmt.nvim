...
function M.load_parsers()
    ...
    vim.treesitter.language.add('html', { path = './tests/parser/html.so' })
    vim.treesitter.language.add('xml', { path = './tests/parser/xml.so' })
    vim.treesitter.language.add('svelte', { path = './tests/parser/svelte.so' })
